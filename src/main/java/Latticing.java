 ...
 */
public class Latticing<K> {

    /** List of all nodes in the lattice */
    private ArrayList<HexCell<K>> nodes;

    /**
     * Default constructor. Creates an empty lattice
     */
    public Latticing() {
        nodes = new ArrayList<HexCell<K>>();
    }
...
//            throw new IllegalArgumentException();
     ...
     */
    public void insert(HexCell<K> neighbor, Dimension inc, Dimension dec) {
        ...
        /** Insert node */
        insert(new HexCell<K>(null, x, y, z));
    }
     ...
     */
    public void flipNodes(HexCell<K> n1, HexCell<K> n2) {
        K key = n1.getKey();
        n1.setKey(n2.getKey());
        n2.setKey(key);
    }
     ...
     */
    public boolean containsNode(HexCell<K> n) {
        for(HexCell<K> node : nodes) {
     ...
     */
    public int getDistance(HexCell<K> a, HexCell<K> b) {
     ...
     */
    public HexCell<K> get(int x, int y, int z) {
        for(HexCell<K> n : nodes) {
    ...
    public void resetVisited() {
        for(HexCell<K> n : nodes) n.visit(false);
    }
